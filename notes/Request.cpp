...
Request::Request(std::string req) : code(200) {
	...
	while (!tmp.eof()) {
		...
		if (pos != std::string::npos) { // verifie si il y a bien un ":" dans la string
			...
			// Check if there is a similar key in map to avoid double same headers
	...
	if (this->type == POST) {
		...
		// Check if content-length exists in headers of the request
		...
		// Check if content-length not equal to the size of the body
...
std::string	Request::checkFile(std::string file) {
	...
	if (point == std::string::npos) { 			// Case: it doesn't find "."
		if (file == "/" && file.size() == 1) { 	// Case : 1ere page Html demandee par le client
			...
			contentType = this->acceptHeader.substr(0, leGrandVainqueur); // fichier ne contient pas forcement une extension donc pas forcement une erreur donc renvoyer le premier contenu dans accept
	...
	// Search if the q=* is in the content of Accept to ignore it
